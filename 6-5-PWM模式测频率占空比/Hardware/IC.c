...
void IC_Init()
{
    ...
    TIM_PWMIConfig(TIM3, &TIM_ICInitStructure); // 将结构体变量交给 TIM_PWMIConfig，配置 TIM3 的输入捕获通道,此函数同时会把另一个通道配置为相反的配置，实现 PWMI 模式
...
uint32_t IC_GetFreq()
{
    return 1000000 / (TIM_GetCapture1(TIM3) + 1); // 测周法得到频率 fx = fc / N，这里不执行 +1 的操作也可，fc = 72M / PSC = 1000000
}

/**
 * @brief 获取占空比
 * @param 无
 * @return uint32_t 返回占空比的百分数
 */
uint32_t IC_GetDuty() 
{
