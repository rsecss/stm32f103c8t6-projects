...
/**
 * @brief PWM 初始化,产生的频率为 1KHz，占空比为 0， 分辨率为 1% 的 PWM 波形
 * @param 无
 * @retval 无
 * @attention
 ...
 * @attention 舵机要求的周期为：20ms，占空比这里舵机高电平时间：0.5ms~2.5ms，所以这里面的 PSC 和 ARR 并不固定，经过尝试设置 PSC + 1 = 72，ARR + 1 = 20k，这样 CCR 设置成500，就是 0.5ms；CCR 设置成 2500， 就是 2.5ms，比较直观
 */
void PWM_Init()
{
    /* 开启时钟 */
    ...
    GPIO_Init(GPIOA, &GPIO_InitStructure); // 将 PA1 引脚初始化为复用推免输出（受外设控制的引脚，均需要配置为复用模式）
    ...
    TIM_TimBaseInitStructure.TIM_Period = 20000 - 1;                 // 计数周期，即 ARR 的值
    TIM_TimBaseInitStructure.TIM_Prescaler = 72 - 1;              // 预分频器，即 PSC 的值
    ...
    TIM_OC2Init(TIM2, &TIM_OCInitStructure);                      // 将结构体变量交给 TIM_OC1Init，配置 TIM2 的输出比较通道 2
...
{
    TIM_SetCompare2(TIM2, Compare); // 设置 CCR2 的值
}
