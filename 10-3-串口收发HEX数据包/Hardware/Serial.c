 ...
 * @brief 串口发送数据包
 * @note 
 * 调用此函数后，Serial_TxPacket 数组的内容将加上包头（FF）包尾（FE）后，作为数据包发送出去
 * @param None
 * @retval None
 */
...
void USART1_IRQHandler()
{
    ...
    static uint8_t pRxState = 0; ///< 定义表示当前接收数据位置的静态变量
    ...
    if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET)
    {
        ...
        {
            while (pRxState < 4) ///< 如果收够4个数据
            {
                Serial_RxPacket[pRxState] = RxState; ///< 将数据存入数据包数组的指定位置
                pRxState++;
            }
            RxState = 2; ///< 置下一个状态
        }
